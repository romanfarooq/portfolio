  ...
  return (
    <header className="z-50 relative">
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 -translate-x-1/2 h-12 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap justify-center items-center g-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            <motion.li
              key={link.name}
            ...
            >
              <Link
                href={link.hash}
                className={clsx(
                  "flex justify-center items-center p-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300",
                  {
